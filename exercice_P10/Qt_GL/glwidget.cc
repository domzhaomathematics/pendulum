...
GLWidget::GLWidget(std::vector<Oscillateur*> a, QWidget *parent)
    : QGLWidget(parent)
    , c(&vue)
{
     c.Collection(a);//on ajoute le support a dessin propre a glwidget a chaque element du systeme pour pouvoir dessiner
     for (size_t i(0); i < c.Collection().size(); i++)
     {
         (c.Collection()[i])->ajoute(&vue);
     }


}

GLWidget::GLWidget(EspacePhase e, QWidget* parent)
    :QGLWidget(parent), c(&vue), e1(e)
{
    //on ajoute vue a l'espace des phases et a son oscillateur
    e1.ajoute(&vue);
    e1.O().ajoute(&vue);
...
void GLWidget::paintGL()
{
  ...
  c.dessine(); //dessine le systeme
  if (e1.Support() != nullptr)      //pour que le widget du systeme ne dessine pas l'espace des phases
                                    //(on veut que le widget de l'espace des phases dessine lespace des phases)
  {
    e1.dessine();
  }
...
void GLWidget::timerEvent(QTimerEvent* event)
{
  ...
  c.evolue(dt, 0.01);
  if (e1.Support() != nullptr)  //on evite de faire evoluer l'attribut espacePhase du widget qui va dessiner le systeme
  {
  e1.evolue(dt, 0.01);
  }
  update();
